...
int bpred_last_result;
int bpred_last_addr;
int bpred_last_normal_addr;
...
#define bpred_ignore_hash 0
#define bpred_size 256
#define bpred_history_level 8
short bpred_buf[bpred_size]={0};
int bpred_hash[bpred_size]={0};

#define bpred_ph_level 4
short bpred_pht[bpred_history_level]={0};//pattern history table
short bpred_twolevel_last_result=0;
...
void arm_ctx_execute(struct arm_ctx_t *ctx)
{
	...
	/* START branch prediction code */
	
	//printf("%X\n", bpred_last_addr);
	
	if(bpred_last_addr > regs->pc || bpred_last_normal_addr != regs->pc){
		is_branch = 1;
		//printf("IS_BRANCH\n");
		
		asARMEmu(arm_emu)->num_branch_uinst++;
		...
		/* Not-taken predictor */
		#ifdef BPRED_NOTTAKEN
		asARMEmu(arm_emu)->num_mispred_branch_uinst++;
		#endif BPRED_NOTTAKEN
		...
		if(bpred_buf[bimod_index] == 0 || (bpred_hash[bimod_index] != bimod_hash && !bpred_ignore_hash))// report as not taken
			asARMEmu(arm_emu)->num_mispred_branch_uinst++;
		if(bpred_hash[bimod_index] != bimod_hash){
			bpred_hash[bimod_index] = bimod_hash;
			bpred_buf[bimod_index] = 1;
		}else{
			bpred_buf[bimod_index]++;
		...
		if(bpred_buf[bimod_index] < (bpred_history_level / 2) || (bpred_hash[bimod_index] != bimod_hash && !bpred_ignore_hash))// report as not taken
			asARMEmu(arm_emu)->num_mispred_branch_uinst++;
		if(bpred_hash[bimod_index] != bimod_hash){
			bpred_hash[bimod_index] = bimod_hash;
			bpred_buf[bimod_index] = 1;
			...
			bpred_buf[bimod_index]++;
			if(bpred_buf[bimod_index]>(bpred_history_level - 1))
				bpred_buf[bimod_index] = bpred_history_level-1;
		}
		#endif BPRED_BIMODAL
		...
		if(bpred_pht[bpred_buf[twolevel_index]] < (bpred_ph_level / 2) || (bpred_hash[twolevel_index] != twolevel_hash && !bpred_ignore_hash))// report as not taken
			asARMEmu(arm_emu)->num_mispred_branch_uinst++;
		
		//feed last time result
		bpred_pht[bpred_buf[twolevel_index]] ++;
		bpred_buf[twolevel_index] = ((bpred_buf[twolevel_index] << 1) + 1) & (bpred_history_level - 1);
		//for(int i=0;i<bpred_size;i++){printf("%d", bpred_pht[bpred_buf[i]]);}printf("\n");
		...
		if(bpred_hash[twolevel_index] == twolevel_hash){
			bpred_pht[bpred_buf[twolevel_index]] --;
			bpred_buf[twolevel_index] = (bpred_buf[twolevel_index] << 1) & (bpred_history_level - 1);
	...
	{
		case(ARM32):
			bpred_last_normal_addr = bpred_last_addr + ctx->inst.info->size;
		break;

		case(THUMB16):
			bpred_last_normal_addr = bpred_last_addr + ctx->inst_th_16.info->size;
		break;

		case(THUMB32):
			bpred_last_normal_addr = bpred_last_addr + 2;
		break;
	}
	/* END branch prediction code */
