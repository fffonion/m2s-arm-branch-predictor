...
void ARMEmuDumpSummary(Emu *self, FILE *f)
{
	...
	branch_acc = (double) (emu->num_branch_uinst -
			emu->num_mispred_branch_uinst) / emu->num_branch_uinst;
	...
	fprintf(f, "Branch Total/Missed = %lld/%lld\n", emu->num_branch_uinst, emu->num_mispred_branch_uinst);
